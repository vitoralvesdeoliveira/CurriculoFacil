...
function getItems(){
    return AsyncStorage.getItem('@curriculofacil:formacoesteste')
            .then(response => {
                if(response)
 ...
 useEffect(() => {
  ...
  okPressed=()=>{
    navigation.navigate('GerarPdf',{navigation,route});
  }
  ...
  { items.map(item => {
   ...
   <IconButton
     ...
     onPress={plusPressed} 
    />
    <Button style={styles.button} onPress={okPressed}><View><Text>OK</Text></View></Button>
  </View>
  )
}