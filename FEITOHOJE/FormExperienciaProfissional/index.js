...
export default FormExperienciaProfissional = ({route,navigation}) => {
  ...
  handleNewFormacao = async() => {  
      ...
      let savedItems = [];
      const response = await AsyncStorage.getItem("@curriculofacil:formacoesteste");
      
      if(response) savedItems = JSON.parse(response);
      savedItems.push(listItem);
      await AsyncStorage.setItem("@curriculofacil:formacoesteste", JSON.stringify(savedItems));
  ...
  <View style={styles.container}>
    ...
    <Text style={styles.header2}>Experiência Profissional</Text>
      ...
      <TextInput style={styles.input}
        ...
        mode='outlined'
        onChangeText={principaisatividades => setText3(principaisatividades)}
      />
    <TouchableOpacity style={styles.button}
      onPress = {handleNewFormacao}
