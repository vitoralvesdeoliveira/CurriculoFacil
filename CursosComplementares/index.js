...
const CursosComplementares = ({ navigation, route }) => {
  function getItems() {
    return AsyncStorage.getItem('@curriculofacil:cursoscomplementares').then(
      (response) => {
  ...
  plusPressed = () => {
    navigation.navigate('FormCursosComplementares');
  };

  //navigation.navigate('GerarPdf',responseData)
  okPressed = () => {
    navigation.navigate('ExpProfissionais');
  };

  return (
    <View style={styles.container}>
      <Text style={styles.header2}>Cursos Complementares</Text>
